function netClusterSet = genKSetOfNetClusterWLifeLen(numNet, netSize, minKClus, maxKClus, minClusSize, maxClusSize, T, minTseg, maxTseg, minRate, maxRate, minPow, maxPow)
	...
	for n =1:numNet
		...
		[netClusterSet{n}.lifetimes, netClusterSet{n}.lifeTypes]= getKLifetimeWRandDistr(T, netClusterSet{n}.K, minTseg, maxTseg, minRate, maxRate, minPow, maxPow);
	end
end

function [lives lifeTypes]= getKLifetimeWRandDistr(T, K, minSeg, maxSeg, minBase, maxBase, minPow, maxPow)
	lives ={};
	lives{K} = {};
	...
	for i = 1:K
		...
		distr = randi([1,3]);
		meanLen = randi([ceil(T/10), floor(T/4*3)]);
		seg = randi([minSeg, maxSeg]);
		[lives{i}, lifeTypes{i}]= genLifetimeWLen(T, K, seg, minBase, maxBase, minPow, maxPow, distr, meanLen);
	end
end