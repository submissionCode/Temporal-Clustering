function netClusterSet = genKSetClusWSynPeriodLife(numNet, netSize, minKClus, maxKClus, minClusSize, maxClusSize, T, minTseg, maxTseg, minRate, maxRate, minPow, maxPow, minAliveRate, maxAliveRate)
	...
	for n =1:numNet
		...
		[netClusterSet{n}.lifetimes, netClusterSet{n}.lifeTypes]= getKSynPeriodicLife(T, netClusterSet{n}.K, minTseg, maxTseg, minRate, maxRate, minPow, maxPow, minAliveRate, maxAliveRate);
	end
end

function [lives lifeTypes]= getKSynPeriodicLife(T, K, minSeg, maxSeg, minBase, maxBase, minPow, maxPow, minAliveRate, maxAliveRate)
	lives ={};
	lives{K} = {};
	lifeTypes ={}; lifeTypes{K} =[];
		...
		[lives{1}, lifeTypes{1}]= genPeriodicLifetime(T, K, seg, alivePercent, minBase, maxBase, minPow, maxPow);
		lifeTypes{1}.syn = true;
	for i = 2:K
		lives{i} = lives{1} .* (rand(size(lives{1}))*4.8+0.2 ); 
		lifeTypes{i} = lifeTypes{1};
	end
end