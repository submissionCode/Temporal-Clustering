%% clusters lifetime length follow different distribution
%% dataset4: generate dynamic networks contains minK~maxK randomly generated clusters. In  this dataset, each cluster has the edge-generating rate within [minBase, maxBase], size within [PA.minClussize, PA.maxClusSize], lifetime segments are randomly generated.
% output to file
...
mkdir(dirname);
%demo
Ts = [1000, 2000, 3000];	%time steps
Ns = [200, 300, 400];	%network size
minK = [10];	% # clusters
maxK = [30];
minBase = [0.2]; %	P
maxBase = [0.8];
