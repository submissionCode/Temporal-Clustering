function [A,Out] = ncpMod(M,r,opts)
%%    [modified to allow modes to  be equal, using opts.modIdx to indicate the mode indices]
...
%           matrices
%           modIdx (array): indices of modes that are constraint to be equal. 
% output:
...
if isfield(opts,'modIdx'); modIdx = opts.modIdx; else modIdx = []; end % add constraint that mode with index in modIdx should be equal
...
estSize = 2*N*prod(nWay)/min(nWay) + prod(nWay);
if estSize< 2 * 500^3  % a tensor can be compute with 1Gb memory, 
    storedata = true;
    sizeN = zeros(1,N);
...
for k = 1:maxit
    ...
    for n = 1:N
        ...
        Gn = Am{n}*Bsq-MB;
        A{n} = max(0,Am{n}-Gn/L(n));  % non-negative 
        if eqCstr && n==modIdx(end)
            for ii = 2:length(modIdx)
                A{modIdx(1)} = A{modIdx(1)} +A{modIdx(ii)};
            end
            A{modIdx(1)} = A{modIdx(1)} ./ length(modIdx);
            for ii = 1:length(modIdx)
                Asq{modIdx(ii)} = A{modIdx(1)};
                Asq{modIdx(ii)} = A{modIdx(ii)}'*A{modIdx(ii)};
            end
        else
            Asq{n} = A{n}'*A{n};
        end 
...
% lambda = ones([1 r]);
% %% normalize all modes 
% for m = 1:length(A.u) %for each mode
%     lambda = max(A.u{m});
%     A.lambda = A.lambda .* lambda';
%     A.u{m} = A.u{m} ./ repmat(lambda, [size(A.u{m}, 1), 1]);
% end
% %% order by norm
% norms = zeros(size(lambda));
...
%         a = a(:) * A.u{m}(:,i)';
%     end
%     norms(i) = A.lambda(i)^2 * sum(a(:).^2);
% end
% [norms sortIdx] = sort(norms,'descend');
% A.lambda = A.lambda(sortIdx);
% %[A.lambda sortIdx] = sort(A.lambda, 'descend');
% for m = 1:length(A.u) %for each mode
%     A.u{m} = A.u{m}(:, sortIdx);
% end
